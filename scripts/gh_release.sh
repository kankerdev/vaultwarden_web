...
LATEST_REMOTE_TAG="v$(git -c 'versionsort.suffix=-' ls-remote --tags --refs --sort='v:refname' https://github.com/kankerdev/vaultwarden_web.git 'v*' | tail -n1 | grep -Eo '[^\/v]*$')"
...
${CONTAINER_CMD} create --name "bw_web_${RELEASE_TAG}" "ghcr.io/kankerdev/vaultwarden_web:${RELEASE_TAG}"
